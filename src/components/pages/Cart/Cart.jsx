...
const Cart = () => {
  const { cartItems, onRemoveCartItems,setCartItems } = useContext(AppContext);
  ...
  const onClickOrder = async() => {
    try{
      ...
      const {data}= await axios.post('https://631076b736e6a2a04eeef849.mockapi.io/orders',{items:cartItems})
      setOrderId(data.id)
      ...
      for (let i=0;i<cartItems.length;i++){
        ...
        await axios.delete(`https://631076b736e6a2a04eeef849.mockapi.io/cartItems/${item.id}`)
        await delay();
      }
    }
    catch(error){
      ...
      <div>
        ...
        {cartItems.length !== 0 && (
          <div className={styles.createOrder}>
            <button onClick={onClickOrder} className={styles.createOrder__btn}>Create order</button>
          </div>
        )}
      </div>
    </main>
